...
const ListMealComponent = () => {
    ...
    const [mealType, setMealType] = useState({ id: 0, typeName: '' });
    ...
    const getAllMeals = () =>{
        MealService.getAllMeals().then((response) =>{
            ...
            if (response.data.length > 0 && response.data[0].mealType) {
                setMealType(response.data[0].mealType);
            }
            


            // setMeals(response.data);
            // if(response.data[0].mealType) {
            //     setMealType(response.data[0].mealType);}
        }).catch(error =>{
            console.log(error);
        })
